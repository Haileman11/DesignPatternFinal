...
public class EmpTree extends JFrame implements TreeSelectionListener {
    ...
    private void makeEmployees() {
        ...
        for (int i = 0; i < 5; i++)
            salesMgr.add(new Employee("Sales " + Integer.toString(i),
        ...
        for (int i = 0; i < 4; i++)
            prodMgr.add(new Employee("Manuf " + Integer.toString(i),
                    25000.0F + (float) (Math.random() - 0.5) * 5000));
        for (int i = 0; i < 3; i++)
            shipMgr.add(new Employee("ShipClrk " + Integer.toString(i),
    ...
    public void valueChanged(TreeSelectionEvent evt) {
        ...
        if (emp != null)
            cost.setText(Float.toString(emp.getSalaries()));
    }
